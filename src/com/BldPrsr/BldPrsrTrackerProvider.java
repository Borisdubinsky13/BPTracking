...
public class BldPrsrTrackerProvider extends ContentProvider {
	public static String TAG = "BldPrsrTrackerProvider";
	...
	public static final String AUTHORITY = "com.BldPrsr.provider.userContentProvider";
	...
	private static class DbAdapter extends SQLiteOpenHelper {
		public DbAdapter(Context context) {
			...
			/*
			 * try { mBackupManager = new BackupManager(context); } catch
			 * (Exception e) { BldPrsrLogger.e(TAG, SubTag + e.getMessage()); }
			 */
		}
		...
		public void onCreate(SQLiteDatabase db) {
			...
			try {
				...
				if (c.getCount() == 0) {
					db.execSQL("CREATE TABLE " + BPUSER_TABLE_NAME
				...
				if (c.getCount() == 0) {
					db.execSQL("CREATE TABLE " + BPSTATUS_TABLE_NAME
							+ " (_id INTEGER PRIMARY KEY AUTOINCREMENT, "
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			BldPrsrLogger.i(TAG, SubTag + "starting an upgrade from "
			...
			try {
				...
				if (from.moveToFirst()) {
					do {
						...
						if (from.getString(from.getColumnIndex("dPrsr"))
								.length() < 4
							...
							vals.put("mDate", from.getString(from
							...
							vals.put("pulse", from.getString(from
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		try {
			...
			case BPSTATUS:
				BldPrsrLogger
						.i(TAG, SubTag + "building query for STATUS table");
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		try {
			...
			switch (sURIMatcher.match(uri)) {
			case BPUSER:
				count = db.update(BPUSER_TABLE_NAME, values, selection,
						selectionArgs);
				break;
			case BPDATA:
				BldPrsrLogger.i(TAG, SubTag + "Updating " + BPDATA_TABLE_NAME);

				count = db.update(BPDATA_TABLE_NAME, values, selection,
						selectionArgs);
				break;
			case BPSTATUS:
				count = db.update(BPSTATUS_TABLE_NAME, values, selection,
						selectionArgs);
				break;
			default:
				throw new IllegalArgumentException("Unknown URI " + uri);
